...
from layouts.home_test import LayoutHomeTest
...
pred_list = dcc.Graph(config={'displayModeBar': False})
...
### HOME ###
home = LayoutHomeTest()
home_layout = home.create(history_cb=filter_box)
...
def display_dropdowns(children, child2):
    items = df.Items.unique()
    for i in range(0, 10):
        curr_item = items[i]
        dfb = df.query(f'Items == "{curr_item}"')
        dfb = pd.DataFrame(dfb.groupby('date').size()).reset_index()
        dfb = dfb.rename({0: 'true_values'}, axis=1)

        new_element = html.Div([
            go.Bar(name='True Sales', x=dfb.date, y=dfb.true_values)
        ])
